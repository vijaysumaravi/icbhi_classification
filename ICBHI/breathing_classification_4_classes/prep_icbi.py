'''
    ...
    Four classes: normal, crackles, wheezes, both
    Authors: 
        - Vijay Ravi
    Date: 2025-01-11
'''
...
def print_data_stats(data_df, train_df, validation_df, test_df):
    ...
    - Number of normal/crackles/wheezes/both utterances
    - Total utterances
    ...
    for df in dfs:
        ...
        n_crackles = len(df[df['utterance_label'] == 'crackles'])
        n_wheezes = len(df[df['utterance_label'] == 'wheezes'])
        n_both = len(df[df['utterance_label'] == 'both'])
        n_total = len(df)
        ...
        stats.append([
            n_patients, 
            f"{n_normal}/{n_crackles}/{n_wheezes}/{n_both}", 
            n_total, 
            f"{duration_hours:.2f}",
...
def get_data_df(audio_files, annotation_files, audio_processing_options):
    '''
        This function creates the dataframe with the following columns:
            ...
            - utterance_label: label for the segment (normal, crackles, wheezes, both)
